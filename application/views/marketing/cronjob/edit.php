...
<div class="page-header page-header-light">
	<div class="page-header-content header-elements-md-inline">
		<div class="page-title d-flex">
			<h4><span class="font-weight-semibold">Update Cron-Job Group</span></h4>
		...
		<div class="card-body">
			...
			<form class="form-validate-jquery" id="article_form" method="post" action="<?php echo site_url('marketing/cronjob/add')?>">
				...
				<div class="form-group row">
					...
					<div class="col-lg-10">
						<input class="form-control" id="pack_name" name="title" placeholder="Group Name" value="<?php echo $data['title']?>" required/>
					</div>
				</div>
				<div class="form-group row">
					...
					<div class="col-lg-10">
						...
						<select type="number" class="form-control" id="quantity" name="quantity">
							<?php foreach ($options as $k=>$v){
								?>
								<option value="<?php echo $k ?>" <?php echo ($k==$data['quantity'])?'selected':''?>><?php echo $v ?></option>
								<?php
							} ?>
						</select>
					</div>
				...
				<div class="form-group row">
					...
					<div class="col-lg-10">
						...
						<select type="number" class="form-control" id="interval" name="interval">
							<?php foreach ($options as $k=>$v){
								?>
								<option value="<?php echo $k ?>" <?php echo ($k==$data['interval'])?'selected':''?>><?php echo $v ?></option>
								<?php
							} ?>
						</select>
					</div>
